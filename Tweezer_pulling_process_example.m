clear;
raw=importdata('mt_example_trace.dat',' ',5);%import data from traces
calb=importdata('Stiffness_example.dat',' ',5);%%import calibration data
...
t1=1;
t2=65.6*1000;%%define the range of interest by time point
stage_x_trace=x_stage(t1:t2)-17.42;%%substract of the voltage offset in x
x_qpd_trace=x_qpd(t1:t2);
stage_y_trace=y_stage(t1:t2)-16.45;%%substract of the voltage offset in y
...
tfe=4.97*1000-t1; %fitting range end time
...
fdf=smoothf(62.6*1000-t1:64.85*1000-t1);
ldf=smoothl(62.6*1000-t1:64.85*1000-t1);
%fdf=smoothf(43600-t1:45.14*1000-t1);
%ldf=smoothl(43600-t1:45.14*1000-t1);
...
plot(smoothl(34000:end),smoothf(34000:end))
hold on;
